      ...
      <nav className={`max-w-7xl mx-auto h-16 md:h-24 glass pointer-events-auto rounded-full md:rounded-[3rem] border transition-all duration-500 flex items-center justify-between px-4 md:px-12 shadow-2xl ${
        ...
        <div className="flex items-center gap-1 md:gap-4 relative shrink-0">
          <button onClick={user ? () => setShowUserMenu(!showUserMenu) : onOpenLogin} 
            ...
            {user ? (
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-full overflow-hidden border-2 border-indigo-500 shadow-lg relative shrink-0">
                <img src={DEFAULT_AVATAR} className="w-full h-full object-cover aspect-square" alt="Avatar" />
              </div>
            ) : (
          ...
          {user && showUserMenu && (
            <>
              ...
              <div className="absolute top-full right-0 mt-6 w-80 bg-[#0f172a] rounded-[2.5rem] p-0 shadow-[0_30px_100px_rgba(0,0,0,0.8)] z-20 border border-white/10 animate-in fade-in slide-in-from-top-4 overflow-hidden">
                <div className="p-8 bg-gradient-to-br from-indigo-600/20 to-transparent border-b border-white/5 relative">
                  <div className="absolute top-4 right-4"><Crown className="w-6 h-6 text-indigo-500/40" /></div>
                  <div className="flex items-center gap-5">
                    <div className="w-16 h-16 rounded-full border-2 border-indigo-500 p-1 bg-slate-900 shadow-xl overflow-hidden shrink-0">
                      <img src={DEFAULT_AVATAR} className="w-full h-full rounded-full object-cover aspect-square" alt="Avatar" />
                    </div>
                    <div className="min-w-0">
                      <p className="text-white font-black text-lg truncate uppercase tracking-tight leading-tight">{user.name}</p>
                      <div className="flex items-center gap-1.5 mt-1">
                        <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                        <p className="text-[10px] text-indigo-400 font-black uppercase tracking-widest">VIP Member</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-8 space-y-4">
                  <div className="flex flex-col gap-3">
                    <div className="p-4 bg-white/5 rounded-2xl border border-white/5 flex flex-col gap-1 hover:bg-white/10 transition-colors">
                      <div className="flex items-center gap-2 text-gray-500">
                        <History className="w-3.5 h-3.5" />
                        <span className="text-[9px] font-black uppercase tracking-widest">Thời hạn hội viên:</span>
                      </div>
                      <span className="text-xs text-indigo-100 font-black tracking-widest pl-5.5 uppercase">{user.expiryDate}</span>
                    </div>
                    
                    <div className="p-4 bg-white/5 rounded-2xl border border-white/5 flex flex-col gap-1 hover:bg-white/10 transition-colors">
                      <div className="flex items-center gap-2 text-gray-500">
                        <Key className="w-3.5 h-3.5" />
                        <span className="text-[9px] font-black uppercase tracking-widest">Mã định danh:</span>
                      </div>
                      <span className="text-xs text-indigo-100 font-black tracking-widest pl-5.5">{user.key.slice(0, 10)}***</span>
                    </div>
                  </div>

                  <button onClick={handleLogout} className="w-full mt-2 py-4.5 rounded-2xl bg-rose-500/10 text-rose-500 text-[11px] font-black uppercase tracking-widest hover:bg-rose-500 hover:text-white transition-all border border-rose-500/20 flex items-center justify-center gap-3 active:scale-95 shadow-lg shadow-rose-900/10">
